...
from lib.phoneme_utils import ASRDatasetProcessor
from lib.base.task import TaskTester
from lib.clf import ProberModel
from lib.probers import Prober, Wav2Vec2Prober
...
class SimpleASRPipeline(TaskTester):
    ...
    def __call__(self,  model2probe: Prober, 
        ...
        for lang in dataset_language:
            if model_path is None: model_path = cc.MODELS_PATH[dataset_name][str(lang)] 
            for feature in features:
                
                for init_strategy in model_init_strategies:
                    title = dataset_name + "_" + _lang(lang) + "_" + feature + "_task_random=" + str(init_strategy) +\
                            "_grads="  +str(enable_grads) + "_variational=" + str(use_mdl) + "_poisoned=" + str(poisoning_ratio)
